...
export class OrdersService {
  ...
  async transitionOrderStatus(transitionDto: TransitionOrderStatusDto) {
    const order = await this.ordersRepository.findOne({
      where: { orderId: transitionDto.orderId },
    });
    if (!order) {
      throw new NotFoundException('Order not found');
    }
    ...
    if (user) {
      ...
      try {
        await this.notificationsService.createNotification({
          ...
          metadata: JSON.stringify({
            rejectionReason,
            attemptNumber: order.paymentProofAttempts,
            attemptsLeft,
          }),
        });
      } catch (err) {
...
<html>
  ...
  async savePaymentProof(
    ...
    if (
      previousStatus === OrderStatus.READY_FOR_BILLING ||
...
<html>
  ...
  async initiatePaymongoCheckout(orderId: number): Promise<{
    ...
    this.logger.log(
      `Initiating PayMongo checkout for Order #${orderId}, Amount: ₱${totalAmount}`,
    );
