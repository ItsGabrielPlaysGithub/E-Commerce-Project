...
export class CategoriesService {
    ...
    async assignRandomCategoriesToProducts(){
        ...
        if (categories.length === 0) {
            throw new NotFoundException('No categories found. Create categories first.');
        }
        ...
        return { message: `Assigned random categories to ${updated.length} products`, count: updated.length };
    }
}
