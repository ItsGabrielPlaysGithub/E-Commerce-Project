...
import { UnauthorizedException, BadRequestException, OnModuleInit } from '@nestjs/common';
...
export class AuthResolver implements OnModuleInit {
    ...
    async login(
        ...
        const ipRateLimit = this.rateLimitService.check(`login:ip:${clientIp}`, 20, 15 * 60 * 1000);
        ...
        const emailRateLimit = this.rateLimitService.check(`login:email:${emailAddress}`, 5, 15 * 60 * 1000);
        ...
        if (isLocked.locked) {
            const readableTime = secondsToReadable(Math.ceil(isLocked.remainingMs! / 1000));
            throw new BadRequestException(
